...
const SimpleFallbackMarkdownRenderer = ({ children }: { children: string }) => {
  ...
  try {
    ...
    return (
      <div 
        className="text-slate-800 leading-relaxed max-w-none"
  ...
  useEffect(() => {
    ...
    const startSync = () => {
      syncTimerRef.current = setInterval(async () => {
        const pendingOps = fileSystem.getPendingOperations()
        ...
        for (const operation of pendingOps) {
          if (hasError) break // 如果有错误，停止同步剩余操作
          ...
          try {
            ...
            switch (operation.type) {
              case 'create':
                if (operation.data.isFolder) {
                  // 文件夹创建
                  await apiService.createFile(taskId, operation.data.filename, '', 'folder')
                } else {
                  await apiService.createFile(taskId, operation.data.filename, operation.data.content)
                }
                break
              case 'save':
                // 检查文件是否存在于后端，如果不存在先创建
                try {
                  await apiService.saveFileContent(taskId, operation.data.filename, operation.data.content)
                } catch (error: any) {
                ...
                fileSystem.saveFile(operation.data.filename)
                break
              case 'delete':
                await apiService.deleteFile(taskId, operation.data.filename)
                break
              case 'rename':
                await apiService.renameFile(taskId, operation.data.oldName, operation.data.newName)
                break
            }
        ...
        // 只有在没有错误时才清除操作队列
        if (!hasError) {
          fileSystem.clearPendingOperations()
        }
      }, 3000) // 每3秒同步一次
    }
